...
// 更新用户操作
  update=async ()=>{
    ...
    const Avatar=AvatarArr.length!=0?AvatarArr[AvatarArr.length-1].name:'avatar-init.png'
    const {getFieldsValue}=this.props.form
    const value=getFieldsValue(['userName','password','vip'])
    const {userName,password,vip}=value
    const result=await reqUpdateUser(_id,Avatar,userName,password,vip);
    if(result.status==0){
    ...
    <Drawer
        ...
        <Form.Item>
          {getFieldDecorator('vip', {
          	initialValue:user.vip
          })(
            <Input
              prefix={<Icon type="rank" />}
              type="text"
              placeholder="是否为VIP"
            />,
          )}
